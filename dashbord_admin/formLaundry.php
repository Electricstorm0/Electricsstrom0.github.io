...
if (isset($_POST['submit'])) {
    function tambah($data, $admin_id) {
        global $conn;

        $id_cust = mysqli_real_escape_string($conn, $_POST['Id_cust']);
        $name = mysqli_real_escape_string($conn, $_POST['name']);
        $room_preference = mysqli_real_escape_string($conn, $_POST['room_preference']);
        $jumlah_item = mysqli_real_escape_string($conn, $_POST['jumlah']);
        $berat = mysqli_real_escape_string($conn, $_POST['berat']);
        $harga = mysqli_real_escape_string($conn, $_POST['harga']);
        $tglMasuk = $_POST['laundry_masuk'];
        $tglSelesai = $_POST['laundry_selesai'];

        $sql = "INSERT INTO laundry (Id_cust, Id_admin, nama, jenis_layanan, jumlah_item, berat, harga, tgl_masuk, tgl_selesai) 
                VALUES ('$id_cust', '$admin_id', '$name', '$room_preference', '$jumlah_item', '$berat', '$harga', '$tglMasuk', '$tglSelesai')";
    ...
    <div class="container mt-5 bg-white shadow-sm p-4">
        ...
        <form action="" method="post">
            ...
            <div class="mb-3">
                ...
                <input type="number" name="Id_cust" placeholder="Enter your ID" required class="form-control">
            </div>
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <input type="number" name="jumlah" placeholder="Enter number of clothes" required class="form-control">
            </div>
            <div class="mb-3">
                <label class="fs-6" for="berat">Berat</label>
                <input type="number" name="berat" placeholder="Enter the weight" required class="form-control">
            </div>
            <div class="mb-3">
                <label class="fs-6" for="harga">Harga</label>
                <input type="number" name="harga" placeholder="Enter the price" required class="form-control">
            </div>
            <div class="mb-3">
                <label class="fs-6" for="tanggal_masuk">Tanggal Masuk</label>
                <input type="date" name="laundry_masuk" required class="form-control">
            </div>
            <div class="mb-3">
                <label class="fs-6" for="tanggal_selesai">Tanggal Selesai</label>
                <input type="date" name="laundry_selesai" required class="form-control">
            </div>
